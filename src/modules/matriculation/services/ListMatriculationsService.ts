...
class ListMatriculationsService{
	...
	public async execute({active, type, orderCreatedAt, username, offset, limit}: IFindAllMatriculationsDTO): Promise<Matriculation[]> {
    const matriculations = await this.matriculationRepository.findAllMatriculations({active, type, orderCreatedAt, username, offset, limit});
