...
class StudentProfileController {
  public async show(request: Request, response: Response): Promise<Response> {
    ...
    const showStudentProfileService = container.resolve(
      ShowStudentProfileService,
    );
  ...
  public async update(request: Request, response: Response): Promise<Response> {
    ...
    const { name, username, password, email, oldPassword, telephone } =
      request.body;

    const updateStudentProfileService = container.resolve(
      UpdateStudentProfileService,
    );
    const student = await updateStudentProfileService.execute({
      id,
      telephone,
      name,
      username,
