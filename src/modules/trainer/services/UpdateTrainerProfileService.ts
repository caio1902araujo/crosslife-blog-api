...
class UpdateTrainerProfileService {
  ...
  public async execute({
    ...
    if (!trainer) {
      throw new AppError('Treinador(a) não existe.', 404);
    }

    const trainerWithUpdatedUsername =
      await this.trainerRepository.findByUsername(username);

    if (
      trainerWithUpdatedUsername &&
      trainerWithUpdatedUsername.id !== trainerId
    ) {
      throw new AppError('Esse nome de usuário já esta em uso', 400);
    }

    const trainerWithUpdatedEmail = await this.trainerRepository.findByEmail(
      email,
    );
